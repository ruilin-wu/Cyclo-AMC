#include <adf.h>
#include <aie_api/aie.hpp>
#include <aie_api/aie_adf.hpp>
#include <aie_api/utils.hpp>

#include "layer4_1_conv2.h"
#include "fam_funcs.h"
#include "parameters.h"

/*──────────────────────── constructor ───────────────────────*/
layer4_1_conv2::layer4_1_conv2(int xoff) : m_xoff(xoff)
{
  aie::set_rounding   (aie::rounding_mode::symmetric_inf);
  aie::set_saturation (aie::saturation_mode::saturate);
}


void layer4_1_conv2::run(
        adf::input_buffer<bfloat16, adf::extents<13824>>& wbuf,
        adf::input_buffer<bfloat16, adf::extents<13824>>& wbuf1,
        input_stream<bfloat16>*                          sin ,
        adf::input_buffer<bfloat16, adf::extents<512>>& residual,
        output_stream<bfloat16>*                         sout)
{
  /* ───────── 型别速记 ───────── */
  using v32   = aie::vector<bfloat16,32>;
  using v16   = aie::vector<bfloat16,16>;
  using v8    = aie::vector<bfloat16, 8>;
  using acc16 = aie::accum <accfloat ,16>;

  constexpr int COLS        = 4;            // 输入宽
  constexpr int COLS_PADDED = COLS + 2;     // 左右各 1
  constexpr int BLKS        = 12;           // 96 IC / 8

  const v16 Z16 = aie::zeros<bfloat16,16>();
  auto rptr = aie::begin_restrict_vector<8>(residual);   // 8192 / 8 = 1024 个 v8

  /* ────── 行缓冲：3 行 × 6 列 × 12 blk ────── */
  alignas(32) v8 row0[COLS_PADDED * BLKS],
                row1[COLS_PADDED * BLKS],
                row2[COLS_PADDED * BLKS];
  v8* rows[3] = {row0,row1,row2};

  /* ────── 读取一整行：96 × 4 v8 / 行 ────── */
  auto load_row = [&](v8* dst)
  {
    /* 左 pad */
    for(int ib = 0; ib < BLKS; ++ib)
      dst[ib] = Z16.extract<8>(0);

    /* 4 列 × 12 块 */
    for(int col = 0; col < COLS; ++col)
    {
      int base = (col + 1) * BLKS;          // +1 跳过左 pad
      for(int ib = 0; ib < BLKS; ++ib)
      {
        v8 v = readincr_v<8>(sin);/* 连续读 8 通道 */
        //writeincr(sout, v);
        dst[base + ib] = v;
      }
    }

    /* 右 pad */
    for(int ib = 0; ib < BLKS; ++ib)
      dst[(COLS_PADDED - 1) * BLKS + ib] = Z16.extract<8>(0);
  };

  /* ────── 预热两行 ────── */
  load_row(row1);   // 输入 0 行
  load_row(row2);   // 输入 1 行

  /* ────── 主循环：4 × 4 ────── */
  for(int r = 0; r < 4; ++r)
  {
    for(int c = 0; c < 4; ++c)
    {
      /* 4 × 4OC 累加器 → 16OC */
      acc16 acc0 = aie::zeros<accfloat,16>(),
            acc1 = aie::zeros<accfloat,16>(),
            acc2 = aie::zeros<accfloat,16>(),
            acc3 = aie::zeros<accfloat,16>(),
            acc4 = aie::zeros<accfloat,16>(),
            acc5 = aie::zeros<accfloat,16>(),
            acc6 = aie::zeros<accfloat,16>(),
            acc7 = aie::zeros<accfloat,16>();


      auto itap0 = aie::begin_restrict_vector<32>(wbuf);
      auto itap1 = aie::begin_restrict_vector<32>(wbuf1);

      /* 12 × 8IC block  (96 IC) */
      for(int ib = 0; ib < BLKS; ++ib)
      {
        /* vin[0..2] : 3 行像素 */
        v32 vin[3];
        for(int kr = 0; kr < 3; ++kr)
        {
          v8  px   = rows[kr][ (1 + c) * BLKS + ib ]; // stride = 1
          v16 pair = aie::concat(px, px);
          vin[kr].insert(0, pair);
          vin[kr].insert(1, pair);
        }

        /* kernel col 0..2 */
        for(int kc = 0; kc < 3; ++kc)
        {
          for(int krw = 0; krw < 3; ++krw)          // 新增 kernel‑row 维度
          {
            /* 每行 32OC → 8 × v32 */
            v32 W0 = *(itap0++);   // OC0‑3
            v32 W1 = *(itap0++);   // OC4‑7
            v32 W2 = *(itap0++);   // OC8‑11
            v32 W3 = *(itap0++);   // OC12‑15
            v32 W4 = *(itap1++);   // OC16‑19
            v32 W5 = *(itap1++);   // OC20‑23
            v32 W6 = *(itap1++);   // OC24‑27
            v32 W7 = *(itap1++);   // OC28‑31

            acc0 = mac_4x8_8x4(vin[krw], W0, acc0);
            acc1 = mac_4x8_8x4(vin[krw], W1, acc1);
            acc2 = mac_4x8_8x4(vin[krw], W2, acc2);
            acc3 = mac_4x8_8x4(vin[krw], W3, acc3);
            acc4 = mac_4x8_8x4(vin[krw], W4, acc4);
            acc5 = mac_4x8_8x4(vin[krw], W5, acc5);
            acc6 = mac_4x8_8x4(vin[krw], W6, acc6);
            acc7 = mac_4x8_8x4(vin[krw], W7, acc7);
          }
        }
      }

      /* ────── ReLU + 写回 ────── */
      v8 lo0 = acc0.to_vector<bfloat16>().extract<8>(0);  // OC0-3
      v8 hi0 = acc1.to_vector<bfloat16>().extract<8>(0);  // OC4-7
      v8 lo1 = acc2.to_vector<bfloat16>().extract<8>(0);  // OC8-11
      v8 hi1 = acc3.to_vector<bfloat16>().extract<8>(0);  // OC12-15
      v8 lo2 = acc4.to_vector<bfloat16>().extract<8>(0);  // OC16-19
      v8 hi2 = acc5.to_vector<bfloat16>().extract<8>(0);  // OC20-23
      v8 lo3 = acc6.to_vector<bfloat16>().extract<8>(0);  // OC24-27
      v8 hi3 = acc7.to_vector<bfloat16>().extract<8>(0);  // OC28-31
      
      
      v8 out0 = aie::shuffle_down_fill(lo0, hi0, 4);
      v8 out1 = aie::shuffle_down_fill(lo1, hi1, 4);
      v8 out2 = aie::shuffle_down_fill(lo2, hi2, 4);
      v8 out3 = aie::shuffle_down_fill(lo3, hi3, 4);

      v8 relu0 = aie::max(aie::add(out0, *(rptr++)), bfloat16(0));
      v8 relu1 = aie::max(aie::add(out1, *(rptr++)), bfloat16(0));
      v8 relu2 = aie::max(aie::add(out2, *(rptr++)), bfloat16(0));
      v8 relu3 = aie::max(aie::add(out3, *(rptr++)), bfloat16(0));

      writeincr(sout, relu0);
      writeincr(sout, relu1);
      writeincr(sout, relu2);
      writeincr(sout, relu3);
      
    }

    /* ────── 行滚动：stride = 1 ────── */
    if(r < 2)
    {
      rows[0] = rows[1];
      rows[1] = rows[2];
      load_row(rows[2]);          // 读入下一原始行
    }
  }
}