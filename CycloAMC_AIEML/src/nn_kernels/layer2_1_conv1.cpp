...
/*──────────────────────── constructor ───────────────────────*/
layer2_1_conv1::layer2_1_conv1(int xoff) : m_xoff(xoff)
{
  ...
  for(int r=0; r<16; ++r){
    for(int c=0; c<16; ++c){
      ...
      for(int ib=0; ib<6; ++ib){
        ...
        /* kernel-column 0..2  ——★ 以下为唯一改动 ★—— */
        for(int kc = 0; kc < 3; ++kc){
          for(int krw = 0; krw < 3; ++krw){          // 新增 krw = kernel row
            /* 8 × v16 : 8IC → 16OC （每 OC‑blk 2 段，避免丢半边） 
            v16 w00_lo = *(itap++);   // OC0‑3  (低半)
            v16 w00_hi = *(itap++);   // OC0‑3  (高半)
            v16 w01_lo = *(itap++);   // OC4‑7
            v16 w01_hi = *(itap++);   // OC4‑7
            v16 w02_lo = *(itap++);   // OC8‑11
            v16 w02_hi = *(itap++);   // OC8‑11
            v16 w03_lo = *(itap++);   // OC12‑15
            v16 w03_hi = *(itap++);   // OC12‑15
            */

            /* 32‑lane 权重向量：完整 8IC×4OC */
            v32 W0 = *(itap++);
            v32 W1 = *(itap++);
            v32 W2 = *(itap++);
            v32 W3 = *(itap++);
      ...
      v8 relu0 = aie::max(out0, bfloat16(0));
      writeincr(sout, relu0);              // 写出最终结果

      v8 relu1 = aie::max(out1, bfloat16(0));
