...
public:
    ...
    mem_to_4_graph_1() {
        ...
        location<buffer>(in_mem) = location<kernel>(k_fam[0]) + relative_offset({.col_offset = 0});
        num_buffers(in_mem) = 2;

        write_access(in_mem.in[0]) = tiling({
            .buffer_dimension = {560},
        ...
        for (int i = 0; i < 4; ++i) {
            read_access(in_mem.out[i]) = tiling({
                .buffer_dimension = {560},
...
public:
    ...
    k_8_stage2_test() {
        ...
        location<buffer>(out_mem) = location<kernel>(k_stage2_1[0]) + relative_offset({ .col_offset = 0 });

        for (int i = 0; i < 4; ++i) {
            write_access(out_mem.in[i]) = tiling({
                .buffer_dimension = {1, 1, 256*4},
                .tiling_dimension = {1, 1, 256},
                .offset = {0, 0, 256 * i},
                .tile_traversal = {
                    { .dimension = 2, .stride = 256, .wrap = 1 },
        ...
        read_access(out_mem.out[0]) = tiling({
            .buffer_dimension = {1,1,256*4},
...
public:
    ...
    k_4_stage2_test() {
        ...
        out_mem = shared_buffer<bfloat16>::create({1, 1, 256*4}, 4, 1);
        num_buffers(out_mem) = 2;
        location<buffer>(out_mem) = location<kernel>(k_stage2_2[0]) + relative_offset({ .col_offset = 0 });
...
private:
    static constexpr int NumKernels = 4;
    kernel k_broad_cast;
    mem_to_4_graph_1<X0, Y0, 0> stage1;
...
public:
    ...
    fam_top() {
        ...
        location<buffer>(scd_mem) = location<kernel>(k_broad_cast) + relative_offset({ .col_offset = 0 });

        for (int i = 0; i < NumKernels; ++i) {
            write_access(scd_mem.in[i]) = tiling({
                .buffer_dimension = {1, 1, 128 * NumKernels * 8},
                .tiling_dimension = {1, 1, 128 * 8},
                .offset = {0, 0, 128 * i * 8},
                .tile_traversal = {
                    { .dimension = 2, .stride = 128 * 8, .wrap = 1 },
                    { .dimension = 0, .stride = 1, .wrap = 1 },
                    { .dimension = 1, .stride = 1, .wrap = 1 }
                }
            });
...
public:
    ...
    fam_top_stem() {
        ...
        num_buffers(scd_mem) = 2;
        location<buffer>(scd_mem) = location<kernel>(k_broad_cast) + relative_offset({ .col_offset = 5 });

        for (int i = 0; i < 4; ++i) {
            write_access(scd_mem.in[i]) = tiling({
                .buffer_dimension = {64, 64, 1},
                .tiling_dimension = {64, 16, 1},
                .offset = {0, 16*i, 0},
                .tile_traversal = {
...
private:
    static constexpr int NuminKernels = 1;
    static constexpr int NumoutKernels = 1;
    kernel k_stem[3];
    kernel k_stem_max[3];
public:
    ...
    stem_alter() {
        ...
        location<buffer>(weight_mem) = location<kernel>(k_stem[0]) + relative_offset({ .col_offset = 0 });
        num_buffers(weight_mem) = 2;

        for (int i = 0; i < 1; ++i) {
            write_access(weight_mem.in[i]) = tiling({
                .buffer_dimension = {216},
                .tiling_dimension = {216},
                .offset = {0},
                .tile_traversal = {
                    { .dimension = 0, .stride = 0, .wrap = 1 }
                }
        ...
        location<buffer>(stemout_mem) = location<kernel>(k_stem[0]) + relative_offset({ .col_offset = 1 });
        num_buffers(stemout_mem) = 2;

        for (int i = 0; i < 3; ++i) {
            write_access(stemout_mem.in[i]) = tiling({
                .buffer_dimension = {32, 32, 24},
                .tiling_dimension = {32, 32, 8},
                .offset = {0, 0, 8 * i},
                .tile_traversal = {
                    { .dimension = 0, .stride = 0, .wrap = 1 },
                    { .dimension = 1, .stride = 0, .wrap = 1 },
...
public:
    ...
    stem() {
        ...
        location<buffer>(stem_mem) = location<kernel>(k_stem[0]) + relative_offset({ .col_offset = 0 });
        num_buffers(stem_mem) = 2;
        for (int i = 0; i < NuminKernels; ++i) {
            write_access(stem_mem.in[i]) = tiling({
...
public:
    ...
    {   
        ...
        location<buffer>(image_mem0) = location<kernel>(k_layer1_0_down[0])    + relative_offset({ .col_offset = -1 });  
        ...
        location<buffer>(image_mem1) = location<kernel>(k_layer1_0_down[0])    + relative_offset({ .col_offset = 0 }); 
        num_buffers(image_mem1) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem1.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
                    .tiling_dimension = {out_W, out_H, out_C/3},             
                    .offset           = {0, 0,  (out_C/3)*i},              
                    .tile_traversal     = {{ .dimension = 0, .stride = 0, .wrap = 1 },{ .dimension = 1, .stride = 0, .wrap = 1 },{ .dimension = 2, .stride =  0, .wrap = 1 }},});
        ...
        location<buffer>(image_mem2) = location<kernel>(k_layer1_0_down[0])    + relative_offset({ .col_offset = 2 }); 
...
public:
    ...
    {   
        ...
        ////////////////////////
        image_mem0 = shared_buffer<bfloat16>::create({in_W, in_H, in_C}, 1, 6);
        location<buffer>(image_mem0) = location<kernel>(k_layer2_0_down[0])    + relative_offset({ .col_offset = -1 });  
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 1; ++i) {
            write_access(image_mem0.in[i]) =
                tiling({.buffer_dimension = {in_W, in_H, in_C},   
        ...
        ////////////////////////
        image_mem1 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 3);
        location<buffer>(image_mem1) = location<kernel>(k_layer2_0_down[0])    + relative_offset({ .col_offset = 0 });  
        num_buffers(image_mem1) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem1.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
        ...
        ////////////////////////
        image_mem2 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 1);
        location<buffer>(image_mem2) = location<kernel>(k_layer2_0_down[0])    + relative_offset({ .col_offset = 2 });  
        num_buffers(image_mem2) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem2.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
...
public:
    ...
    {   
        ...
        ////////////////////////
        image_mem0 = shared_buffer<bfloat16>::create({in_W, in_H, in_C}, 1, 6);
        location<buffer>(image_mem0) = location<kernel>(k_layer2_1_down[0])    + relative_offset({ .col_offset = -1 });  
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 1; ++i) {
            write_access(image_mem0.in[i]) =
                tiling({.buffer_dimension = {in_W, in_H, in_C},   
        ...
        ////////////////////////
        image_mem1 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 3);
        location<buffer>(image_mem1) = location<kernel>(k_layer2_1_down[0])    + relative_offset({ .col_offset = 0 });  
        num_buffers(image_mem1) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem1.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
        ...
        ////////////////////////
        image_mem2 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 1);
        location<buffer>(image_mem2) = location<kernel>(k_layer2_1_down[0])    + relative_offset({ .col_offset = 2 });  
        num_buffers(image_mem2) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem2.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
...
public:
    ...
    {   
        ...
        ////////////////////////
        image_mem0 = shared_buffer<bfloat16>::create({in_W, in_H, in_C}, 1, 6);
        location<buffer>(image_mem0) = location<kernel>(k_layer3_0_down[0])    + relative_offset({ .col_offset = -1 });  
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 1; ++i) {
            write_access(image_mem0.in[i]) =
                tiling({.buffer_dimension = {in_W, in_H, in_C},   
        ...
        ////////////////////////
        image_mem1 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 3);
        location<buffer>(image_mem1) = location<kernel>(k_layer3_0_down[0])    + relative_offset({ .col_offset = 0 });  
        num_buffers(image_mem1) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem1.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
        ...
        ////////////////////////
        image_mem2 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 1);
        location<buffer>(image_mem2) = location<kernel>(k_layer3_0_down[0])    + relative_offset({ .col_offset = 2 });  
        num_buffers(image_mem2) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem2.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
...
public:
    ...
    {   
        ...
        ////////////////////////
        image_mem0 = shared_buffer<bfloat16>::create({in_W, in_H, in_C}, 1, 6);
        location<buffer>(image_mem0) = location<kernel>(k_layer4_0_down[0])    + relative_offset({ .col_offset = -1 });  
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 1; ++i) {
            write_access(image_mem0.in[i]) =
                tiling({.buffer_dimension = {in_W, in_H, in_C},   
        ...
        ////////////////////////
        image_mem1 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 3);
        location<buffer>(image_mem1) = location<kernel>(k_layer4_0_down[0])    + relative_offset({ .col_offset = 0 });  
        num_buffers(image_mem1) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem1.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
        ...
        ////////////////////////
        image_mem2 = shared_buffer<bfloat16>::create({out_W, out_H, out_C}, 3, 1);
        location<buffer>(image_mem2) = location<kernel>(k_layer4_0_down[0])    + relative_offset({ .col_offset = 2 });  
        num_buffers(image_mem2) = 2;
        for (int i = 0; i < 3; ++i) {
            write_access(image_mem2.in[i]) =
                tiling({.buffer_dimension = {out_W, out_H, out_C},   
