...
public:
    ...
    k_8_stage2_test() {
        for (int i = 0; i < NumKernels; ++i) {
            k_stage2[i] = kernel::create_object<fam_stage2>(base_index + 2*i);
            source   (k_stage2[i])      = "fam_stage2.cpp";
            runtime<ratio>(k_stage2[i]) = 0.9;
            connect(din, k_stage2[i].in[0]);            
        }

        location<kernel>(k_stage2[0])        = tile(xoff+0, yoff+0);
        location<kernel>(k_stage2[1])        = tile(xoff+0, yoff+1);
        location<kernel>(k_stage2[2])        = tile(xoff+0, yoff+2);
        location<kernel>(k_stage2[3])        = tile(xoff+0, yoff+3);
        location<kernel>(k_stage2[4])        = tile(xoff+1, yoff+0);
        location<kernel>(k_stage2[5])        = tile(xoff+1, yoff+1);
        location<kernel>(k_stage2[6])        = tile(xoff+1, yoff+2);
        location<kernel>(k_stage2[7])        = tile(xoff+1, yoff+3);               
        for (int i = 0; i < NumKernels; ++i) {
           
            location<buffer>(k_stage2[i].out[0]) = location<kernel>(k_stage2[i]);
            location<stack >(k_stage2[i])        = location<kernel>(k_stage2[i]);
        }

        /* ──────────────────────────────
        * out_mem → dout  整块 8192 顺序读
        * ──────────────────────────────*/
        out_mem = shared_buffer<bfloat16>::create({1, 1, 64*2*NumKernels}, NumKernels, 1);
        num_buffers(out_mem) = 2;
        location<buffer>(out_mem) = location<kernel>(k_stage2[0])    + relative_offset({ .col_offset = 0 });  // 只支持列偏移

        for (int i = 0; i < NumKernels; ++i) {
            write_access(out_mem.in[i]) =
                tiling({
                    .buffer_dimension = {1, 1, 64*2*NumKernels},   // 整块缓冲
                    .tiling_dimension = {1, 1, 64*2},      // 一次写 8 点
                    .offset           = {0, 0, 64*2 * i},  // 各核起始 0/8/16/24
                    .tile_traversal   = {
                        { .dimension = 2, .stride = 64*2, .wrap = 1 }, // 交织步长 32
                        { .dimension = 0, .stride = 1,  .wrap = 1 },
                        { .dimension = 1, .stride = 1,  .wrap = 1 }
                    }
                });
            connect(k_stage2[i].out[0], out_mem.in[i]);
            }
        ...
        * ──────────────────────────────*/
        read_access(out_mem.out[0]) =
            tiling({
                .buffer_dimension = {1,1,64*2*NumKernels},
                .tiling_dimension = {1,1,64*2*NumKernels},
                .offset           = {0,0,0},
                .tile_traversal   = { {2,64*2*NumKernels,1},{0,1,1},{1,1,1} }
            });
...
template<
    ...
    int  NK  = 1                            // stage-2 个数
>
class layer_block1_3out : public adf::graph
{
private:
...
public:
    ...
    adf::port<input>  input_weight_1_conv1;    
    adf::port<input>  input_weight_1_down;
    adf::port<input>  input_weight_1_conv2;
    */
    ...
    shared_buffer<bfloat16> image_mem3;
    shared_buffer<bfloat16> image_mem4;
    */
    ...
    {   
        ...
        ////////////////////////
        image_mem0 = shared_buffer<bfloat16>::create({32, 32, 24}, 1, 6);
        location<buffer>(image_mem0) = location<kernel>(k_layer1_0_down[0])    + relative_offset({ .col_offset = 0 });  // 只支持列偏移
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 1; ++i) {
            write_access(image_mem0.in[i]) =
                tiling({.buffer_dimension = {32, 32, 24},   // 整块缓冲  W×H×C
                    .tiling_dimension = {32, 32, 24},              // 一次写 **1 行**
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            = tiling({.buffer_dimension   = {32, 32, 24},
                .tiling_dimension   = {32, 32, 24},
                .offset             = {0, 0,  0},
                .tile_traversal     = {
                    { .dimension = 0, .stride = 0, .wrap = 1 },
        ...
        connect<>(image_mem0.out[3], k_layer1_0_down[0].in[1]);
        connect<>(image_mem0.out[4], k_layer1_0_down[1].in[1]);
        connect<>(image_mem0.out[5], k_layer1_0_down[2].in[1]);

        ////////////////////////
        //0_conv1 to mem1
        ////////////////////////
        image_mem1 = shared_buffer<bfloat16>::create({32, 32, 24}, 3, 3);
        location<buffer>(image_mem0) = location<kernel>(k_layer1_0_down[0])    + relative_offset({ .col_offset = 0 });  // 只支持列偏移
        num_buffers(image_mem0) = 2;
        for (int i = 0; i < 3; ++i) {
        ...
        //3 output
        ////////////////////////
        connect<>(k_layer1_0_conv2[0].out[0], output[0]);
        connect<>(k_layer1_0_conv2[1].out[0], output[1]);
        connect<>(k_layer1_0_conv2[2].out[0], output[2]);
...
public:
    ...
    {   
        ...
        connect<>(image_mem0.out[3], k_layer2_1_down[0].in[1]);
        connect<>(image_mem0.out[4], k_layer2_1_down[1].in[1]);
        connect<>(image_mem0.out[5], k_layer2_1_down[2].in[1]);
...
public:
    ...
    {   
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            location<kernel>(k_layer4_0_conv1[i]) = tile(X0 , Y0+3 + i);
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            location<kernel>(k_layer4_0_conv2[i]) = tile(X0+1 , Y0 +0+ 2*i);
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            = tiling({.buffer_dimension   = {in_W, in_H, in_C},
                ...
                .offset             = {0, 0,  0},
                .tile_traversal     = {{ .dimension = 0, .stride = 0, .wrap = 1 }, { .dimension = 1, .stride = 0, .wrap = 1 },{ .dimension = 2, .stride =  0, .wrap = 1 }},});}
        connect<>(image_mem0.out[0], k_layer4_0_conv1[0].in[1]);
        connect<>(image_mem0.out[1], k_layer4_0_conv1[1].in[1]);
        connect<>(image_mem0.out[2], k_layer4_0_conv1[2].in[1]);
        ...
        //mem1 to 0_conv2
        ////////////////////////            
        connect<>(image_mem1.out[0], k_layer4_0_conv2[0].in[1]);
        connect<>(image_mem1.out[1], k_layer4_0_conv2[1].in[1]);
        connect<>(image_mem1.out[2], k_layer4_0_conv2[2].in[1]);

        ////////////////////////
        //0_residual to 0_conv2
        ////////////////////////
        for (int i = 0; i < 3; ++i) {
            connect<>(k_layer4_0_down[i].out[0], k_layer4_0_conv2[i].in[2]);
        }
