    ...
    for(int r=0;r<32;++r){
      ...
      for (int ib = 0; ib < 3; ++ib) {
        ...
        for (int kc = 0; kc < 3; ++kc) {
          ...
          /* 按 OC 切半：低 16lane → OC0-3， 高 16lane → OC4-7 */
          v32 w0 = aie::concat(w.extract<16>(0), Z32.extract<16>(0));
          v32 w1 = aie::concat(w.extract<16>(1), Z32.extract<16>(0));

          acc0 = mac_4x8_8x4(vin[0], w0, acc0);
          acc0 = mac_4x8_8x4(vin[1], w0, acc0);
          acc0 = mac_4x8_8x4(vin[2], w0, acc0);

          acc1 = mac_4x8_8x4(vin[0], w1, acc1);
          acc1 = mac_4x8_8x4(vin[1], w1, acc1);
          acc1 = mac_4x8_8x4(vin[2], w1, acc1);
        }
      } /* ib */
      ...
      v8 out  = aie::max(aie::shuffle_down_fill(low, high, 4),
                         bfloat16(0));
      writeincr(sout, out);                               // 8 值/像素
    } /* c */
